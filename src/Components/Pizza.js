...
export default function Pizza({pizzas}) {
    ...
    return (
        ...
        <div style={{margin:'40px'}} className="shadow-lg p-3 mb-5 bg-white rounded">
             <h4 id="h4" style={{marginBottom:"0px",color:"	Magenta"}}>{pizzas.name}</h4>
           <div onClick={handleShow} id="card">
        <img style={{borderRadius:"20px"}} src={pizzas.image}  alt="pic" height="200px" width="200px"/>
           </div>
    ...
    <button className="btn" onClick={addtocart}><Link to="/card" style={{textDecoration:"none",color:"rgb(218, 200, 200)"}}> BUY NOW 
    </Link></button>
                  </div>
                </div>
                </div>
